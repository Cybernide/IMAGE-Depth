static class HapticBox {
  ...
  public HapticBox(float x, float y, float l, float w) {
    ...
    topRight = new PVector(x+w,y);  // Double l/w for compatibility with create_box which doubles the l/w for no apparent reason
    bottomRight = new PVector(x+w,y+l);
    bottomLeft = new PVector(x,y+l);
