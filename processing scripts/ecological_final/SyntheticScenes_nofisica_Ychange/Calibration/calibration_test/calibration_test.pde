...
void keyPressed() {
  if (key == 'c') {
    println("X Position of Cursor is : " + (posEE.x * pixelsPerMeterx) + "   Y Position of Cursor is: " + (posEE.y * pixelsPerMetery));
  }
  else if (key == 'w') {
    saveTable(log, "log.csv");
  }
    ...
    println("New Pixel per Meter value along X: " + ((pos2.x - pos1.x)* pixelsPerMeterx)/0.1 + "   New Pixel per Meter value along Y: " + ((pos2.y - pos1.y)* pixelsPerMetery)/0.1);
  }
