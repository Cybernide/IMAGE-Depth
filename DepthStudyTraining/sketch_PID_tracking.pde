...
final float       worldPixelWidthf                    = 1000;
final float         worldPixelHeightf                 = 650;
float x_m,y_m;
...
// define a flag to toggle path tracking
boolean trace = false;
float xr = 0.5;
float yr = 0.5;
...
int looptime = 500;
// flag to toggle random loop times
boolean randomLoopTime = false;
...
void setup(){
  ...
  ping1 = new SoundFile(this, "ping1.mp3");
  ping2 = new SoundFile(this, "ping2.mp3");
  ping3 = new SoundFile(this, "ping3.mp3");
  ping4 = new SoundFile(this, "ping4.mp3");
...
public void TraceCircle(int theValue)  {
  // trace a circle with radius 0.5
  // since this is only called once per button press, set a flag to toggle tracing
  
}

public void RandomLoopTime(int theValue)  {
  // toggle the flag to randomize loopTime
  randomLoopTime = !randomLoopTime;
}

public void TraceCircularPath()  {
  // we model the circle using Polar coordinates, r and theta
  // r = radius = 0.5
  // theta = circle_angle, which is time-changing (we choose to add 0.01 to the angle at each time-step)
  xr = radius * cos(circle_angle);
  yr = radius * sin(circle_angle);
  // update the circle_angle at each timestep
  circle_angle = circle_angle + angle_delta;
  // reset if a loop is completed
  if (circle_angle > 360.0f)  {
    circle_angle -= 360.0f;
  }
}
...
void mousePressed(){
    xr = mouseX/worldPixelWidthf; 
    yr = mouseY/worldPixelHeightf;
    print("mouse position is: " + xr + " , " + yr + " ");
}
...
while(1==1) {
    ...
    if(haplyBoard.data_available()){
      ...
      x_m = xr * worldPixelWidthf-500; 
      y_m = yr * worldPixelHeightf;//mouseY;
...
void update_animation(float th1, float th2, float xE, float yE){
  ...
  x_m = xr * worldPixelWidthf; //*300+500; 
      //println(x_m + " " + mouseX);")
      y_m = yr* worldPixelHeightf; //300+350;//mouseY;
  pushMatrix();
  translate(x_m, y_m);
  shape(target);
  popMatrix();
